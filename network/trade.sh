...
function printHelp () {
  echo "Usage: "
  echo "  trade.sh up|down|restart|generate|upgrade [-c <channel name>] [-f <docker-compose-file>] [-i <imagetag>] [-o <logfile>]"
...
function networkClean () {
  ...
  rm -rf channel-artifacts/*.block channel-artifacts/*.tx crypto-config ./crypto-config/ channel-artifacts/carrierorg.json
...
function replacePrivateKey () {
  ...
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/EXPORTER_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
  cd crypto-config/peerOrganizations/importerorg.trade.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/IMPORTER_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
  cd crypto-config/peerOrganizations/carrierorg.trade.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CARRIER_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
  cd crypto-config/peerOrganizations/regulatororg.trade.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/REGULATOR_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml

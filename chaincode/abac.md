...
The following set of instructions are meant to be an extension of **Access Control** section of **Chapter 4: Designing a Data and Transaction Model with Golang**.  The section on access control from the book covers the concept of `attribute based access control` or ABAC for short.  This git page is meant to provide a complementary practical exercise.  
  
If you have followed the instructions within the chapter, you should currently have a running network in development mode.  Furthermore, you should have a running chaincode container and have succesfully invoked the requestTrade function using the `peer chaincode inovke` command.

A quick `docker ps` should return a list of 5 docker container:  
 * cli
 * chaincode
 * peer
 * ca
...
The first thing we will do is create additional identities to test the new attribute.  As you may remember, the network pre-built identities where created using `cryptogen`.  While this tool is really useful to create identities for test network, you need to use fabric-ca in order to generate certificates with custom attributes.

In order to create the new identity we have a custom script that we provide.  Let's first run it and we will then look at what it did:
...
The first command enroll the administrator.  The admin/adminpw combo has been defined by the cryptogen process.  Note the parameter `--mspdir` that is used to store the resulting certificates under the `/root/.fabric-ca-client/admin` folder.
...
Once the identity is registered, normally the enrollment credentials would be sent to the owner and the last command would be issued to enroll the user.  Note that the user must specify the attributes that he wants to include in the certificate.  Of course this is an exercise so the enrollment follows immediately after the `register` command.  Also note that the certificates will now be stored in a folder `/root/.fabric-ca-client/importer`

Finally, we move the signcert into a directory called admincerts.  This essemtially tells the `peer` command to treat this identity as an admin.  However, it does not confer administrative privilege at the network level.

With the additional identities created we can now modify the chaincode to make use of it.
...
In tradeWorkflow.go requestTrade function locate the marker `// ADD TRADELIMIT RETRIEVAL HERE` and add this code:
...
````
This code, leverage the function defined in accessControlUtils.go to retrieve the trade limit.  If the tradelimit is not found then the code sets a default value of 1,000,000.00.  This is done to make sure that the remainder of the code in subsequent chapters will not be impacted by this attribute.
...
This is the power of the dev mode.  No need to reinstall the chaincode or re-instantiate.  The system is ready to test our new code.  Note that in a multi-org system, the discrepancies between chaincode version would automatically be picked up since not all organization would be running the same code... dev mode is only useful and functional for a single organization.
...
The result should be an Error and look like:
